...
# # ==== Range 主导包装：把某个通道复制为 out_ch 通道 ====
# import torch
# from torch.utils.data import Dataset
#
# class RangeDominantWrapper(Dataset):
#     def __init__(self, base, src_ch=1, out_ch=2):
#         """
#         src_ch: 在 v5 张量里 0=R, 1=G, 2=B；你的 range 在 G → 用 1
#         out_ch: 模型期望输入通道数。你的首层 groups=2 → 设 2
#         """
#         self.base = base
#         self.src_ch = src_ch
#         self.out_ch = out_ch
...
#
#     # 透传原 dataset 的属性（尤其是 collate_fn、indices 等）
#     def __getattr__(self, name):
#         return getattr(self.base, name)
#
#     def __getitem__(self, i):
#         out = self.base[i]                   # 常见返回：(img, labels, paths, shapes, ...)
#         img = out[0]                         # torch.float32 [C,H,W] (RGB)
#         chosen = img[self.src_ch:self.src_ch+1, :, :]
#         img = chosen.repeat(self.out_ch, 1, 1)  # -> [out_ch,H,W]
#         if isinstance(out, (list, tuple)):
#             out = list(out); out[0] = img; return tuple(out)
#         out["img"] = img; return out
# # ==== End ====
...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        ...
        # 仅训练集包；若希望 val 也用 range-only，去掉 if
        if isinstance(prefix, str) and prefix.lower().strip().startswith('train'):
            dataset = RangeDominantWrapper(dataset, src_ch=1, out_ch=2)
...
class LoadImagesAndLabels(Dataset):
    ...
    #     return self
    from utils.augmentations import two_channel_u8_augment
    def __getitem__(self, index):
        ...
        # reduce to two channels
        img = img[:,:, 0:2]

        if self.augment:
            if not hasattr(self, "_aug_dbg_cnt"):
                self._aug_dbg_cnt = 0
            self._aug_dbg_cnt += 1

            do_log = (self._aug_dbg_cnt % 200) == 0  # 每200次记录一次
            if do_log:
                before = img.copy()

            img = two_channel_u8_augment(img, p=1.0)

            if do_log:
                diff = float(np.mean(np.abs(img.astype(np.int16) - before.astype(np.int16))))
                with open("aug_debug.log", "a", encoding="utf-8") as fp:
                    fp.write(f"[AUG] pid={os.getpid()} diff={diff:.3f} path={self.im_files[index]}\n")
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, i):
        raise RuntimeError("DATALOADER NEW VERSION HIT")
